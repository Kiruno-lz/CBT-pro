...
pub fn vwap(
    ...
    if prices.len() != volumes.len() {
        return Err(IndicatorError::InvalidParameter("prices and volumes must have same length".to_string()));
    }
    if prices.is_empty() {
    ...
    for i in 0..prices.len() {
        ...
        if cum_vol == Decimal::ZERO {
            return Err(IndicatorError::CalculationError("cumulative volume is zero".to_string()));
        }
