...
fn decimal_sqrt(value: Decimal) -> Result<Decimal, IndicatorError> {
    let f = value.to_f64()
        .ok_or_else(|| IndicatorError::CalculationError("cannot convert Decimal to f64".to_string()))?;
    let sqrt_f = f.sqrt();
    if sqrt_f.is_nan() || sqrt_f.is_infinite() {
        return Err(IndicatorError::CalculationError("sqrt result is invalid".to_string()));
    }
    Decimal::from_f64(sqrt_f)
        .ok_or_else(|| IndicatorError::CalculationError("cannot convert f64 to Decimal".to_string()))
}
