...
fn test_always_long_default_params() {
    ...
    assert_eq!(
        always_long.default_params,
        serde_json::json!({}),
        "AlwaysLong should have empty default params"
    );
}
...
fn test_bollinger_bands_default_params() {
    ...
    let bb = strategies
        .iter()
        .find(|s| s.id == "bollinger_bands")
        .unwrap();
    assert_eq!(
        bb.default_params.get("period").unwrap().as_u64().unwrap(),
        20
    );
    assert_eq!(
        bb.default_params.get("std_dev").unwrap().as_str().unwrap(),
        "2.0"
    );
}
...
fn test_always_long_param_definitions() {
    ...
    let always_long = strategies.iter().find(|s| s.id == "always_long").unwrap();
    assert!(
        always_long.param_definitions.is_empty(),
        "AlwaysLong should have no param definitions"
    );
}
...
fn test_rsi_macd_param_definitions() {
    ...
    let rsi_macd = strategies.iter().find(|s| s.id == "rsi_macd").unwrap();
    assert_eq!(
        rsi_macd.param_definitions.len(),
        4,
        "RSI MACD should have 4 param definitions"
    );

    let names: Vec<&str> = rsi_macd
        .param_definitions
        .iter()
        .map(|p| p.name.as_str())
        .collect();
...
fn test_bollinger_bands_param_definitions() {
    ...
    let bb = strategies
        ...
        .unwrap();
    assert_eq!(
        bb.param_definitions.len(),
        2,
        "Bollinger Bands should have 2 param definitions"
    );
...
fn test_breakout_param_definitions() {
    ...
    let breakout = strategies.iter().find(|s| s.id == "breakout").unwrap();
    assert_eq!(
        breakout.param_definitions.len(),
        2,
        "Breakout should have 2 param definitions"
    );

    let lookback = breakout
        .param_definitions
        .iter()
        .find(|p| p.name == "lookback")
        .unwrap();
...
fn test_create_always_long() {
    ...
    let mut strategy = (always_long.create)(
        "BTC-USDT".to_string(),
        Decimal::from(1),
        serde_json::json!({}),
    )
    ...
    assert_eq!(
        signals.len(),
        1,
        "AlwaysLong should emit exactly one signal"
    );
...
fn test_create_ema_crossover_with_defaults() {
    ...
    let mut strategy = (ema.create)(
        "BTC-USDT".to_string(),
        Decimal::from(1),
        ema.default_params.clone(),
    )
    .unwrap();
    ...
    assert!(
        signals.is_empty(),
        "EMA Crossover with insufficient data should return empty"
    );
}
...
fn test_create_ema_crossover_with_custom_params() {
    ...
    let mut strategy =
        (ema.create)("ETH-USDT".to_string(), Decimal::from(2), custom_params).unwrap();
    let signals = strategy.on_bar(&make_minimal_context());
    assert!(
        signals.is_empty(),
        "EMA Crossover with insufficient data should return empty"
...
fn test_create_rsi_macd_with_defaults() {
    ...
    let mut strategy = (rsi_macd.create)(
        "BTC-USDT".to_string(),
        Decimal::from(1),
        rsi_macd.default_params.clone(),
    )
    .unwrap();
    let signals = strategy.on_bar(&make_minimal_context());
    assert!(
        signals.is_empty(),
        "RSI MACD with insufficient data should return empty"
    );
}
...
fn test_create_bollinger_bands_with_defaults() {
    ...
    let mut strategy = (bb.create)(
        "BTC-USDT".to_string(),
        Decimal::from(1),
        bb.default_params.clone(),
    )
    .unwrap();
    let signals = strategy.on_bar(&make_minimal_context());
    assert!(
        signals.is_empty(),
        "Bollinger Bands with insufficient data should return empty"
    );
}
...
fn test_create_breakout_with_defaults() {
    ...
    let mut strategy = (breakout.create)(
        "BTC-USDT".to_string(),
        Decimal::from(1),
        breakout.default_params.clone(),
    )
    .unwrap();
    let signals = strategy.on_bar(&make_minimal_context());
    assert!(
        signals.is_empty(),
        "Breakout with insufficient data should return empty"
    );
}
